...
OCR は RapidOCR の `PP-OCRv5 server` モデルを PyTorch CUDA バックエンドで使用します。
このモデルは日本語、中国語、英語を含む文字認識に対応しています。

RapidOCR のモデルは派生イメージのビルド時に取得されます。実行時にモデルを
ダウンロードしないため、起動後のOCRはネットワーク接続に依存しません。
